// @todo I’m still not entirely sure how settings work for the aggregate schema--how does it know which schema to install? Why doesn’t the `field.widget.settings.*` schema get created? Presumably this is managed in core somewhere, but probably requires more figuring out how how settings work in general. May be beyond the scope of this ebook.
